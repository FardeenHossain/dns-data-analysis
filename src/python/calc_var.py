...
import prog_var
import strain_rate
from config import dx, ix_end, ix_start, iy_end, iy_start, iz_end, iz_start
...
def calc_data(data_file1_path, data_file2_path):
    ...
    [lambda1, lambda2, lambda3, rr1, rr2, rr3] = strain_rate.calc_strain_rate_eig(
...
def calc_pdf(c_half, s_d, lambda1, lambda2, lambda3):
    ...
    [
        ...
        lambda3_pdf,
        lambda3_pdf_bin,
    ] = pdf.calc_strain_rate_pdf(c_half, lambda1, lambda2, lambda3)
...
def calc_jpdf(c_half, s_d, lambda1, lambda2, lambda3, k):
    [
        ...
        lambda3_jpdf_bin_x,
        lambda3_jpdf_bin_y,
    ] = pdf.calc_strain_rate_jpdf(c_half, s_d, lambda1, lambda2, lambda3)

    [s_d_c_half_jpdf, s_d_c_half_jpdf_bin_x, s_d_c_half_jpdf_bin_y] = (
        pdf.calc_disp_speed_prog_var_jpdf(c_half, s_d)
    )

    [s_d_k_jpdf, s_d_k_jpdf_bin_x, s_d_k_jpdf_bin_y] = (
        pdf.calc_disp_speed_curvature_jpdf(c_half, s_d, k)
    )
...
def calc_cond_mean(c_half, s_d, lambda1, lambda2, lambda3):
    ...
    [bin_s_d, lambda1_cond_mean, lambda2_cond_mean, lambda3_cond_mean] = (
        pdf.calc_lambda_c_cond_mean(c_half, s_d, lambda1, lambda2, lambda3)
    )
