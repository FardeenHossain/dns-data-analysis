...
import h5py
import numpy as np
from config import data_path, flame, in_path, nx_c, ny_c, nz_c, position
...
def write_reduced_data():
    ...
    for i in range(0, len(data_files1)):
        ...
        [
            ...
            rho_half,
            k,
        ] = calc_var.calc_data(data_file1_path, data_file2_path)
...
def read_reduced_data():
    ...
    # Initialise arrays
    [c_half_full, s_d_full, mag_g_c_full, k_full] = read_disp_speed(data_files[0])
    [lambda1_full, lambda2_full, lambda3_full, rr1_full, rr2_full, rr3_full] = (
        read_lambda(data_files[0])
    )
    ...
    return [c_half_full, s_d_full, lambda1_full, lambda2_full, lambda3_full, k_full]
...
def write_plot_data():
    ...
    [
        ...
        lambda3_pdf,
        lambda3_pdf_bin,
    ] = calc_var.calc_pdf(c_half, s_d, lambda1, lambda2, lambda3)
    ...
    [
        ...
        lambda3_jpdf_bin_y,
        s_d_c_half_jpdf,
        s_d_c_half_jpdf_bin_x,
        s_d_c_half_jpdf_bin_y,
        s_d_k_jpdf,
        s_d_k_jpdf_bin_x,
        s_d_k_jpdf_bin_y,
    ] = calc_var.calc_jpdf(c_half, s_d, lambda1, lambda2, lambda3, k)
    ...
    [
        ...
        lambda2_cond_mean,
        lambda3_cond_mean,
    ] = calc_var.calc_cond_mean(c_half, s_d, lambda1, lambda2, lambda3)
    ...
    write_disp_speed_prog_var_jpdf(
        s_d_c_half_jpdf, s_d_c_half_jpdf_bin_x, s_d_c_half_jpdf_bin_y
    )
    write_disp_speed_curvature_jpdf(s_d_k_jpdf, s_d_k_jpdf_bin_x, s_d_k_jpdf_bin_y)
...
def write_lambda_jpdf(lambda_jpdf, lambda_jpdf_bin_x, lambda_jpdf_bin_y, subscript):
    ...
    for i in range(0, len(lambda_jpdf[:, 0, 0])):
        for j in range(0, len(lambda_jpdf[i, :, 0])):
            for k in range(0, len(lambda_jpdf[i, j, :])):
                file.write(
                    f"{lambda_jpdf_bin_x[j]} {lambda_jpdf_bin_y[k]} "
                    f"{lambda_jpdf[i, j, k]}\n"
                )
        file.write("\n")
...
def write_disp_speed_curvature_jpdf(s_d_k_jpdf, s_d_k_jpdf_bin_x, s_d_k_jpdf_bin_y):
...
def write_disp_speed_prog_var_jpdf(
    s_d_c_half_jpdf, s_d_c_half_jpdf_bin_x, s_d_c_half_jpdf_bin_y
):
