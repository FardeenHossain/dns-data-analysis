...
def pdf2d(q1, q2, bin_edges_pdf1, bin_edges_pdf2):
    ...
    pdf, x_edges, y_edges = np.histogram2d(
        q1_flat, q2_flat, bins=(bin_edges_pdf1, bin_edges_pdf2), density=True
    )
...
def cond_pdf2d(q1, q2, c, bin_edges_pdf1, bin_edges_pdf2, bin_c_cond, d_bin_c_cond):
    bin_c_pdf1 = 0.5 * (bin_edges_pdf1[:-1] + bin_edges_pdf1[1:])
    bin_c_pdf2 = 0.5 * (bin_edges_pdf2[:-1] + bin_edges_pdf2[1:])
    ...
    for j in range(0, nc):
        ...
        pdf, x_edges, y_edges = np.histogram2d(
            q1_cond_flat,
            q2_cond_flat,
            bins=(bin_edges_pdf1, bin_edges_pdf2),
            density=True,
        )
...
def cond_mean_c(q1, q2, c, bin_q2_cond, bin_c_cond, d_bin_q2_cond, d_bin_c_cond):
    nb = len(bin_q2_cond)
    nc = len(bin_c_cond)
    ...
    for j in range(0, nc):
        ...
        for i in range(0, nb):
            cond = np.absolute(q2_cond_flat - bin_q2_cond[i]) < d_bin_q2_cond / 2.0
