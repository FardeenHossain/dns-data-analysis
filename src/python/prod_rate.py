...
import my_stat
import numpy as np
from config import data_path, o2_b, o2_u
...
def main():
    ...
    [prod_rate_cond_jpdf, prod_rate_cond_jpdf_bin_x, prod_rate_cond_jpdf_bin_y] = (
        calc_prod_rate_cond_jpdf(c_half, s_d, prod_rate)
    )

    [s_d_cond_jpdf, s_d_cond_jpdf_bin_x, s_d_cond_jpdf_bin_y] = (
        calc_disp_speed_cond_jpdf(c_half, s_d, s_d_r)
    )

    [prod_rate_jpdf, prod_rate_jpdf_bin_x, prod_rate_jpdf_bin_y] = (
        calc_prod_rate_prog_var_jpdf(c_half, prod_rate)
    )

    print("Writing data...\n")
    write_prod_rate_pdf(prod_rate_pdf, prod_rate_bin_pdf)

    write_prod_rate_prog_var_jpdf(
        prod_rate_jpdf, prod_rate_jpdf_bin_x, prod_rate_jpdf_bin_y
    )

    write_prod_rate_cond_jpdf(
        prod_rate_cond_jpdf, prod_rate_cond_jpdf_bin_x, prod_rate_cond_jpdf_bin_y
    )

    write_disp_speed_cond_jpdf(s_d_cond_jpdf, s_d_cond_jpdf_bin_x, s_d_cond_jpdf_bin_y)
    ...
    for i in range(0, len(prod_rate[:, 0, 0])):
        for j in range(0, len(prod_rate[i, :, 0])):
            for k in range(0, len(prod_rate[i, j, :])):
                s_d_r[i, j, k] = (prod_rate[i, j, k] / rho_half[i, j, k]) / mag_g_c[
                    i, j, k
                ]
...
def calc_prod_rate_pdf(c_half, prod_rate):
    ...
    # Calculate PDF
    prod_rate_pdf, prod_rate_bin_pdf = my_stat.cond_pdf(
        prod_rate, c_half, bin_edges_pdf, bin_c_cond, d_bin_c_cond
    )
...
def calc_prod_rate_prog_var_jpdf(c_half, prod_rate):
    ...
    # Calculate JPDF
    [prod_rate_jpdf, prod_rate_jpdf_bin_x, prod_rate_jpdf_bin_y] = my_stat.pdf2d(
        prod_rate, c_half, prod_rate_bin_edges_pdf, c_half_bin_edges_pdf
    )
...
def calc_disp_speed_cond_jpdf(c_half, s_d, s_d_r):
    ...
    # Calculate JPDF
    [s_d_cond_jpdf, s_d_cond_jpdf_bin_x, s_d_cond_jpdf_bin_y] = my_stat.cond_pdf2d(
        s_d_r,
        s_d,
        c_half,
        s_d_r_bin_edges_pdf,
...
def write_prod_rate_prog_var_jpdf(
    prod_rate_jpdf, prod_rate_jpdf_bin_x, prod_rate_jpdf_bin_y
):
...
def write_disp_speed_cond_jpdf(s_d_cond_jpdf, s_d_cond_jpdf_bin_x, s_d_cond_jpdf_bin_y):
