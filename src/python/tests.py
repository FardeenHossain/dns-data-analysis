...
import numpy as np
import pdf
from config import data_path, flame, position
...
def cond_mean_test():
    ...
    [c_half, s_d, mag_g_c, k_m] = files.read_disp_speed(data_files[0])
    ...
    for i in range(1, 9):
        [c_half_i, s_d_i, mag_g_c_i, k_m_i] = files.read_disp_speed(data_files[i])
...
def curvature_test():
    ...
    gridToVTK("./output", x, y, z, cellData={"c": c_half[:, :, :], "k_m": k_m[:, :, :]})
