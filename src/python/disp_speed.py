import numpy as np
from config import dx, nx_c, ny_c, nz_c
    ...
    for i in range(0, nx_c):
        for j in range(0, ny_c):
            for k in range(0, nz_c):
                conv_u[i, j, k] = (
                    (u_half[i + 1, j, k] + u_half[i, j, k]) / 2 * g_cx[i, j, k]
                )
                conv_v[i, j, k] = (
                    (v_half[i, j + 1, k] + v_half[i, j, k]) / 2 * g_cy[i, j, k]
                )
                conv_w[i, j, k] = (
                    (w_half[i, j, k + 1] + w_half[i, j, k]) / 2 * g_cz[i, j, k]
                )
