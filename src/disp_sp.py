...
mag_gPROG = np.zeros([Nx_c, Ny_c, Nz_c])
...
for i in range(0, Nx_c):
    for j in range(0, Ny_c):
        for k in range(0, Nz_c):
            convU[i, j, k] = (U_half[i + 1, j, k] +
                              U_half[i, j, k]) / 2 * gCx[i, j, k]
            convV[i, j, k] = (V_half[i, j + 1, k] +
                              V_half[i, j, k]) / 2 * gCy[i, j, k]
            convW[i, j, k] = (W_half[i, j, k + 1] +
                              W_half[i, j, k]) / 2 * gCz[i, j, k]

mag_gPROG = (gCx ** 2.0 + gCy ** 2.0 + gCz ** 2.0) ** 0.5

print(f"mag_gPROG: {mag_gPROG.shape}")
...
condition = np.absolute(PROG_half - 0.73) < (0.2 / 2.0)
...
for i in range(0, Nx_c):
    for j in range(0, Ny_c):
        for k in range(0, Nz_c):
            centerU[i, j, k] = (U_half[i + 1, j, k] + U_half[i, j, k]) / 2
            centerV[i, j, k] = (V_half[i, j + 1, k] + V_half[i, j, k]) / 2
            centerW[i, j, k] = (W_half[i, j, k + 1] + W_half[i, j, k]) / 2
...
[pdf_disp_sp_cond, bin_pdf_disp_sp_cond] = mystat.cond_pdf(
    disp_sp_PROG[:, :, :],
    PROG_half[:, :, :], bin_edges_pdf, bin_c_cond, d_bin_c_cond)
...
plt.contourf(disp1_jpdf_bin, lam1_jpdf_bin, lam1_disp_sp_PROG_jpdf, cmap=cmap,
...
plt.contourf(disp2_jpdf_bin, lam2_jpdf_bin, lam2_disp_sp_PROG_jpdf, cmap=cmap,
...
plt.contourf(disp3_jpdf_bin, lam3_jpdf_bin, lam3_disp_sp_PROG_jpdf, cmap=cmap,
             vmin=0.075e-6, levels=10)
